...
function historyReading(st,stats){
 ...
 if(stats.clean.length===1)return `Existe un único registro cargado para ${st.nombre}. No se calcula tendencia ni línea temporal hasta incorporar nuevas observaciones.`;
 const parts=[];
 const trend=stats.deltaPrev==null?'sin comparación previa':stats.deltaPrev>0?'con tendencia ascendente':stats.deltaPrev<0?'con tendencia descendente':'sin variación respecto al registro anterior';
...
function renderHistory(preselect){
 ...
 html+=`<div class="panel"><div class="history-toolbar"><div><label>Estación</label><select id="histStation" data-change="history-station">${opts}</select></div><div><label>Desde</label><input type="date" id="histDesde" value="${desdeVal}" data-change="history-custom"></div><div><label>Hasta</label><input type="date" id="histHasta" value="${hastaVal}" data-change="history-custom"></div><button type="button" class="btn" data-action="history-custom-apply">Consultar</button></div></div>`;
...
function drawChart(data,stats,id,view=HISTORY_VIEW){
 ...
 const containerW=Math.max(420,(viewport?.clientWidth||el.clientWidth)-30);
 const pointGap=data.length>180?34:data.length>90?42:data.length>45?52:68;
 ...
 const svg=`<svg class="chart-canvas" width="${w}" height="285" viewBox="0 0 ${w} 285" preserveAspectRatio="none">${refs.join('')}<text x="${w-p}" y="24" text-anchor="end" fill="#aebdc5" font-size="10">${htxt}</text><path d="${path}" fill="none" stroke="#4fb3a9" stroke-width="3" vector-effect="non-scaling-stroke"/><g>${circles}</g>${xLabels}</svg>`;
 el.innerHTML=`<div class="chart-scroll" id="chart-scroll" style="min-width:${w}px">${svg}</div>`;
 const sc=viewport;
