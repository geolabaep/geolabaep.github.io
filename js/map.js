...
function updateMapLegend(){if(!state.mapLegendEl)return;const contexto=state.mapMode==='daily'?'Cambio respecto al registro anterior':`Tendencia comparada a ${state.mapTrendPeriod} días`;state.mapLegendEl.innerHTML=`<div class="map-legend-head"><div class="map-legend-kicker">Interpretación del mapa</div><div class="map-legend-title">${contexto}</div></div><div class="map-legend-section"><div class="map-legend-section-title">Dirección</div><div class="map-legend-directions"><div class="map-legend-direction"><span class="map-legend-arrow up">↑</span><span>Sube</span></div><div class="map-legend-direction"><span class="map-legend-arrow down">↓</span><span>Baja</span></div><div class="map-legend-direction"><span class="map-legend-arrow stable">→</span><span>Estable</span></div></div></div><div class="map-legend-section"><div class="map-legend-section-title">Actualidad del dato</div><div class="map-legend-quality"><div class="map-legend-quality-row"><span class="legend-state-dot current"></span><span>Actualizada</span></div><div class="map-legend-quality-row"><span class="legend-state-dot recent"></span><span>Desactualizada hasta 10 días: conserva la última dirección conocida</span></div><div class="map-legend-quality-row"><span class="legend-state-question">?</span><span>Más de 10 días sin actualización</span></div><div class="map-legend-quality-row"><span class="legend-state-dot empty"></span><span>Sin dato</span></div></div></div>`}
...
function popupSparkline(id){const serie=historial(id).slice(-10);if(!serie||serie.length<2)return '<div class="popup-mini-trend"><div class="popup-mini-trend-title">Tendencia reciente</div><div class="popup-note">Sin datos suficientes para graficar.</div></div>';return `<div class="popup-mini-trend"><div class="popup-mini-trend-head"><div class="popup-mini-trend-title">Tendencia reciente</div><div class="popup-mini-trend-range">${serie.length} registros</div></div>${renderMiniSpark(serie)}</div>`}
...
function renderSidebar(){
 ...
 const contextLabel=state.mapMode==='daily'?'cambio diario':`tendencia ${state.mapTrendPeriod} días`;
