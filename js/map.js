...
const PARAGUAY_BOUNDS=[[-27.75,-62.75],[-19.15,-54.15]];
function applyInitialMapView(map){
 if(window.matchMedia('(max-width:760px)').matches)map.fitBounds(PARAGUAY_BOUNDS,{padding:[10,10],maxZoom:6,animate:false});
 else map.setView([-24.4,-57.6],6);
}
function initMap(){const target=qs('#map');if(!target)return false;if(typeof window.L==='undefined'){target.innerHTML='<div class="map-fallback"><b>Mapa no disponible</b><span>No se pudo cargar Leaflet. El panel de estaciones y los demás módulos siguen operativos.</span></div>';return false}try{state.map=L.map('map');applyInitialMapView(state.map);L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png',{maxZoom:18,attribution:'© OpenStreetMap'}).addTo(state.map);renderMarkers();addMapLegend();setTimeout(()=>state.map?.invalidateSize(false),0);return true}catch(error){console.error('[Observatorio] No fue posible iniciar el mapa.',error);target.innerHTML='<div class="map-fallback"><b>Mapa no disponible</b><span>El panel de estaciones y los demás módulos siguen operativos.</span></div>';state.map=null;return false}}
